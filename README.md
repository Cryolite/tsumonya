...
The implementation is based on an idea of computing mahjong scores only by simple array indexing. In other words, this library pre-computes the fu (符) and fan (飜) for **ALL POSSIBLE MAHJONG WINNING HANDS** and stores them in a huge array. In this way, it completely omits the computationally expensive processes normally required for mahjong score calculation, such as menzi (面子) decomposition, calculation of fu, determination of Yakus (役), calculation of fan, logic to resolve ambiguities in menzi decomposition for an winning hand based on the highest point principle (高点法), and others.
...
The `Situation` enum type is used to specifiy hand types (yaku, 役) that cannot be determined from just the hand, melded tiles, and the winning tile. Multiple objects of the type `Situation` can be combined with `operator|`.
